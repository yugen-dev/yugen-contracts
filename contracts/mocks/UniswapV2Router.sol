...
library UniswapV2Library {
    using SafeMathUniswapV2 for uint256;
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IUniswapV2Pair(
            address(pairFor(factory, tokenA, tokenB))
        ).getReserves();
...
// File contracts/uniswapV2/interfaces/IUniswapV2Router.sol
...
// File contracts/uniswapV2/libraries/Initializable.sol
...
// File contracts/uniswapV2/libraries/EIP712Base.sol
...
// File contracts/uniswapV2/libraries/NativeMetaTransaction.sol
...
contract UniswapV2Router is IUniswapV2Router, NativeMetaTransaction {
    using SafeMathUniswapV2 for uint256;
    ...
    constructor(address _factory, address _WETH) {
        factory = _factory;
        WETH = _WETH;
        _initializeEIP712("UniswapV2Router");
    }
    ...
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransfer(token, to, IERC20UniswapV2(token).balanceOf(address(this)));
        IWETH(WETH).withdraw(amountETH);
        TransferHelper.safeTransferETH(to, amountETH);
    }
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            uint256 amountOutput;
            {
                ...
                (uint256 reserveInput, uint256 reserveOutput) = input == token0
                    ? (reserve0, reserve1)
                    : (reserve1, reserve0);
                amountInput = IERC20UniswapV2(input).balanceOf(address(pair)).sub(reserveInput);
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        uint256 balanceBefore = IERC20UniswapV2(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20UniswapV2(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
            "UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    }
