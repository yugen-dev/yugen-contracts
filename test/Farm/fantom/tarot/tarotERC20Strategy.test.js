...
describe("Tarot ERC20 Strategy - Yugen Farm Contract", function () {
  before(async function () {
    ...
    this.tarotSupplyVaultStrategyInstance = await TarotSupplyVaultStrategy.deploy(
      ConstructorParams.USDC.asset,
      ConstructorParams.USDC.supplyVault,
    ...
    this.assetTokenInstance = ERC20Mock.attach(ConstructorParams.USDC.asset);
  ...
  it("should pause deposit/withdraw when contract is paused", async function () {
    ...
    const depositAmount = getBigNumber(100, 6);
  ...
  it("should correctly deposit in tarot supply vault strategy", async function () {
    console.log(
      `USDC Balance for the user before deposit - `,
      await this.assetTokenInstance.balanceOf(this.depositor.address)
    );
    this.depositAmount = getBigNumber(100, 6);
    await this.farmInstance.connect(this.depositor).deposit(0, this.depositAmount, false);
    ...
    console.log(
      `USDC Balance for the user after deposit - `,
      await this.assetTokenInstance.balanceOf(this.depositor.address)
    );
  });
  ...
  it("should disable strategy to rescue funds and handle deposit/withdraw etc.", async function () {
    //New deposit
    await this.farmInstance.connect(this.depositor).deposit(0, this.depositAmount, true);
    let userInfo = await this.farmInstance.userInfo(0, this.depositor.address);
    let totalInputTokensStakedInStrategy = await this.farmInstance.getLpTokenAmount(0);
    let totalInputTokensStaked = (await this.farmInstance.poolInfo(0)).totalInputTokensStaked;
    expect(userInfo.amount).to.equal(this.depositAmount);
    ...
    await expect(
      this.farmInstance.connect(this.depositor).deposit(0, this.depositAmount, true)
